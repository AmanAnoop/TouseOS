...
import { OfficerDashboardShell } from "@/components/dashboard/officer-dashboard-shell";
...
export default async function DashboardPage() {
  ...
  return (
    ...
    >
      ...
      {isOfficer ? (
        <OfficerDashboardShell
          orgId={orgId}
          widgets={{
            deadlines: <UpcomingDeadlines items={deadlineItems} />,
            stats_primary: (
            ...
            health: (
              <Card>
                <CardHeader title="Health score" icon={<Heart size={16} />} action={<Link href="/health" className="text-xs text-greek-600 hover:underline">Details</Link>} />
            ...
            attendance_chart: <AttendanceTrendChart points={attendanceTrend} />,
            engagement_chart: <EngagementTrendChart points={engagementTrend} />,
            events: (
              <Card>
            ...
            chapter_links: isGreekOrg(orgType) ? (
              <Card>
          ...
          } satisfies Partial<Record<DashboardWidgetId, ReactNode>>}
        />
      ) : (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <StatCard title="Upcoming events" value={events.length} icon={<Calendar size={18} />} />
