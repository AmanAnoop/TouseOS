...
export async function PATCH(request: Request) {
  ...
  const { id, chatMessage, task, taskToggle, budgetLine, guest, riskItem, senderName, workspaceStatus } = await request.json();
  ...
  if (workspaceStatus && ["active", "completed", "cancelled"].includes(workspaceStatus)) {
    updates.status = workspaceStatus;
  }
