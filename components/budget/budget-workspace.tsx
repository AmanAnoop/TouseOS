...
interface BudgetWorkspaceProps {
  /** Hide duplicate page header when embedded in unified finance page */
  embedded?: boolean;
}

export function BudgetWorkspace({ embedded = false }: BudgetWorkspaceProps) {
  ...
  if (!loading && !canViewBudget) {
    return (
      <Alert type="warning" title="Access restricted" description="You need treasurer or officer budget permissions to view budget lines." />
    );
  }

  return (
    <div className={embedded ? "space-y-5" : "ds-page-stack"}>
      {!embedded && (
      <PageHeader
