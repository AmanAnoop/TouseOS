...
export async function loadYearbookExportData(
  supabase: SupabaseClient,
  orgId: string,
  orgName: string,
): Promise<YearbookExportData> {
  ...
  const [eventsRes, photosRes, albumsRes, sectionsRes] = await Promise.all([
    supabase
      ...
      .eq("org_id", orgId)
      .gte("starts_at", semesterStart.toISOString())
      .order("starts_at"),
    supabase
      .from("photos")
  ...
  const albumByEvent = new Map(
    albums.filter((a) => a.event_id).map((a) => [String(a.event_id), a]),
  );
